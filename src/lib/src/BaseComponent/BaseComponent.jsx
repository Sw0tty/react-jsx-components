...
class BaseComponent extends Component {
    ...
    _checkPropsRules(propsRules) {
        ...
        for (let prop of propsRules) {
            ...
            if ('minimum' in prop) {
                propError = this._checkMinimumProp(prop);
            }

            if ('maximum' in prop) {
                propError = this._checkMaximumProp(prop);
            }
    ...
    _checkMinimumProp(prop) {
        if (typeof(prop.minimum) !== 'number') {
            return this._getErrorBlock("Minimum value must be type of 'number'.");
        }

        if (typeof(this.props[prop.name]) !== 'number' && !Array.isArray(this.props[prop.name])) {
            return this._getErrorBlock(`For checking prop on minimum, value of prop '${prop.name}' must be type of Number or Array.`);
        }

        if (typeof(this.props[prop.name]) === 'number' && this.props[prop.name] < prop.minimum) {
            return this._getErrorBlock(`Value of prop '${prop.name}' less then minimum of '${this.props[prop.name]}'.`);
        }

        if (Array.isArray(this.props[prop.name]) && this.props[prop.name]?.length < prop.minimum) {
            return this._getErrorBlock(`Length of prop '${prop.name}' less then minimum of '${this.props[prop.name]}'.`);
        }

        return;
    }
    _checkMaximumProp(prop) {
        if (typeof(prop.maximum) !== 'number') {
            return this._getErrorBlock("Maximum value must be type of 'number'.");
        }

        if (typeof(this.props[prop.name]) !== 'number' && !Array.isArray(this.props[prop.name])) {
            return this._getErrorBlock(`For checking prop on maximum, value of prop '${prop.name}' must be type of Number or Array.`);
        }

        if (typeof(this.props[prop.name]) === 'number' && this.props[prop.name] > prop.maximum) {
            return this._getErrorBlock(`Value of prop '${prop.name}' greater then maximum of '${this.props[prop.name]}'.`);
        }

        if (Array.isArray(this.props[prop.name]) && this.props[prop.name]?.length > prop.maximum) {
            return this._getErrorBlock(`Length of prop '${prop.name}' greater then maximum of '${this.props[prop.name]}'.`);
        }
    ...
    _getErrorBlock(message) {
        return <div className="jsxrc-basecomponent-error-container"><span>{message ?? "ERROR"}</span></div>;
    }
    _getRequiredSign() {
        return <span className="jsxrc-basecomponent-required-sign">*</span>;
    }
    render() {

...
class ComboBoxPage extends BaseMethods {
    ...
    render() {
        return(<Playground title="ComboBox"
                pComponent={<ComboBox selectedItem={4} actionShadowColor={this.state.shadowColorAction} invalid={this.state.invalid} actionBorderColor={this.state.borderColorAction} caption={this.state.caption} valueKey="id" captionKey="caption" width={this.state.width} required={this.state.required} disabled={this.state.disabled} items={this.state.items} />}
                componentProps={<>
                ...
                componentDocumentation={[
                    ...
                    {name: "selectedItem", required: false, dataType: "any", description: "Find value in items by valueKey and select value as default."},
                    ...
                    {name: "required", required: false, dataType: "boolean", description: "Component required state. Default is 'false'."},
                    {name: "invalid", required: false, dataType: "boolean", description: "Component invalid state. Default is 'false'."},
                ...
                example={`<ComboBox caption="${this.state.caption}" valueKey="id" captionKey="caption" width={${this.state.width}} invalid={${this.state.invalid}} required={${this.state.required}} disabled={${this.state.disabled}} items={[${this.state.items.map(el => { return `{ id: ${el.id}, caption: '${el.caption}' }` })}]}  onReturnData={{ func: this.setData }} />`}
            />);
    }
}
